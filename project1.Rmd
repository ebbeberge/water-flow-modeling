...
data_preparation = function(df, years, days) {
  ...
  df = df %>% dplyr::filter(format(dato, "%Y") > (2019-years-5))
  ...
  train = df %>% dplyr::filter(format(dato, "%Y") < 2016)
  test = df %>% dplyr::filter(format(dato, "%Y") >= 2016) 
...
```{r}
years = 40 # max 60, min 5 (since 4 years are automatically used for testing)
days = 7
