...
```{r}
# Adding months as a factor
eggafoss <- eggafoss %>%
  dplyr::mutate(eggafoss, month = lubridate::month(dato))
eggafoss$month <- factor(eggafoss$month)
```

We can create dummy variables for the month variable (and any other variable if needed):

```{r}
# Creating Dummy Variables for the month
eggafoss <- fastDummies::dummy_cols(eggafoss, select_columns = "month")
```
