...
lasso.fit = glmnet(x = x_train, y = y_train, alpha = 1, standardize = TRUE, lambda = lambdagrid, nfolds = 10)
cvlasso.fit = cv.glmnet(x_train, y_train, alpha = 1, standardize = TRUE, lambda = lambdagrid, nfolds = 10)
...
for (i in c(0, cvlasso.fit$lambda.min, cvlasso.fit$lambda.1se, 0.1, 0.2)) {
  lasso = glmnet(x = x_train, y = y_train, alpha = 1, standardize = TRUE, lambda = i, nfolds = 10)
...
```{r}
resmat = cbind(coef(lasso.fit, s = cvlasso.fit$lambda.min), coef(lasso.fit, s = cvlasso.fit$lambda.1se), coef(lasso.fit, s = 0.2))
colnames(resmat) = c("lambda min", "lambda 1se","lambda 0.2")
print(resmat)
```


Discuss the possible data leakage and CV...
...
ggplot(test, aes(x = dato)) +
  geom_line(aes(y = vannføring)) +
  geom_line(aes(y = bestsub.pred), color = "blue", linetype = "twodash")
```
